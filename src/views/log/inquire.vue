<template>
	<div class="log-inquire-view-wrapper view-wrapper">
		<div class="inner-header-wrapper">
			<el-col :span="20">
				<el-form inline label-width="80px" :model="inquireFormData" @submit.native.prevent>
		...
		<div class="inner-container-wrapper banner-list">
			<div class="inner-container-title">操作日志</div>
	...
	methods: {
		...
		deleteCheck() {
			this.selectColle.length &&
				this.$confirm('确定删除？')
					.then(() => {
						this.selectColle.forEach((e) => {
							this.dataTable.forEach((item, i) => {
								if (item.name1 === e.name1) {
