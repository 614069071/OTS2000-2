			...
			<el-row class="inner-item">
				...
				<div class="editor-item">
					...
					<el-form-item label="升级模块" prop="module" required>
						<el-select v-model="putFormData.module" :disabled="isDisabled" placeholder="请选择">
							<el-option value="all" label="整包"></el-option>
						</el-select>
					</el-form-item>

					<el-form-item label="应用ID" prop="applicationId" required>
						<el-select :disabled="isDisabled" v-model="putFormData.applicationId" placeholder="请选择">
							<el-option :value="1" label="HD12"></el-option>
						</el-select>
					</el-form-item>

					<el-form-item label="设备型号" prop="modelId" required>
						<el-select :disabled="isDisabled" v-model="putFormData.modelId" placeholder="请选择">
				...
				<div class="editor-item">
					<el-row>
						<el-form-item label="版本号" prop="version" required>
							<el-input :disabled="isDisabled" v-model="putFormData.version" maxlength="25" placeholder="请输入版本号" />
						</el-form-item>
					</el-row>

					<el-row>
						<el-form-item label="包名" prop="packageNames" required>
							<el-input :disabled="isDisabled" v-model="putFormData.packageNames" maxlength="25" placeholder="请输入包名" />
						</el-form-item>
					</el-row>
					...
					<el-row>
						<el-form-item label="上传软件包" :error="uploadFileErrorMes">
							<div class="upload-bin-wrapper" :class="{disabled:isDisabled}">
								<input id="upload_bin" :disabled="isDisabled" type="file" accept=".bin" hidden @change="uploadBinFileChange">
					...
					<el-row>
						<el-form-item label="是否发布">
							<el-switch :disabled="isDisabled" inactive-value="0" active-value="1" v-model="putFormData.isRelease" @change="switchRelease"></el-switch>
						</el-form-item>
					</el-row>
				</div>
			</el-row>
...
export default {
	...
	computed: {
		isDisabled() {
			return !!parseInt(this.putFormData.isRelease);
		},
	},
	activated() {
		console.log('activated ota-editor');
		const otaCache = utils.storages.get('OTACACHE');
		const { size, upgradeUrl } = otaCache;
		this.putFormData = otaCache;
		this.binFileInfo = { size, upgradeUrl };
	},
	methods: {
		...
		async saveForm() {
			...
			if (this.putFormData.isRelease === '1') {
				...
				flag && this.submitForm();
			}
		},
		cancelForm() {
			this.$router.push('/ota');
			this.$store.commit('DELETE_CACHE_BREAD', '/ota/editor');
			utils.storages.set('OTACACHE', {});
		},
		staticSubmit() {
